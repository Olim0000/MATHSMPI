...
\item déterminant $\det$, rang $\rg$, matrice $\mathrm{Mat}(u,\mathcal{B})$ d'un endomorphisme dans une base $\mathcal{B}$ ;
...
\begin{proof}
Le polynôme caractéristique $\chi_A(X)=\det(A-X I_n)$ est un polynôme de degré $n\geq 1$ à coefficients dans $\mathbb{C}$. D'après le théorème de d'Alembert--Gauss, il admet au moins une racine $\lambda\in\mathbb{C}$, et $\det(A-\lambda I_n)=0$ signifie précisément que $\lambda$ est valeur propre de $A$ (proposition~\ref{prop:critere-vp}).
\end{proof}
