...
\begin{prop}[Réduction aux suites réelles]
\label{prop:norme-suite-reelle}
Soient $(u_n)\in E^{\mathbb{N}}$ et $\ell\in E$. Alors
\[u_n\xrightarrow[n\to+\infty]{}\ell \text{ dans } E \iff \|u_n-\ell\|\xrightarrow[n\to+\infty]{}0 \text{ dans } \mathbb{R}\]
\end{prop}

\begin{proof}
Posons $\alpha_n=\|u_n-\ell\|$ : c'est une suite \emph{réelle} positive. Par définition de la convergence dans $\mathbb{R}$,
\[\alpha_n\to 0 \iff \forall\varepsilon>0,\ \exists N_0,\ \forall n\geq N_0,\ |\alpha_n-0|\leq\varepsilon\]
et $|\alpha_n|=\alpha_n=\|u_n-\ell\|$ puisque $\alpha_n\geq 0$. La condition de droite est donc mot pour mot la définition~\ref{defi:convergence} de « $u_n\to\ell$ ».
\end{proof}

C'est ce résultat, d'apparence anodine, qui permet de ramener l'étude des suites d'un e.v.n. à celle des suites réelles : c'est lui qu'on invoque implicitement chaque fois qu'on majore $\|u_n-\ell\|$ par une suite réelle tendant vers $0$.
...
\emph{Second terme :} $M\|\ell_1\|_E$ est une constante et $\|y_n-\ell_2\|_F\to 0$ (proposition~\ref{prop:norme-suite-reelle}), donc ce terme tend vers $0$ (proposition~\ref{prop:scalaire-limite} appliquée dans $\mathbb{R}$).
...
donne $\big\|B(x_n,y_n)-B(\ell_1,\ell_2)\big\|_G\to 0$, c'est-à-dire la convergence annoncée (proposition~\ref{prop:norme-suite-reelle}).
\end{proof}
